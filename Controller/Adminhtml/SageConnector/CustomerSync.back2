...
class CustomerSync extends Action
{
    ...
    public function execute()
    {
        ...
        $select = $connection->select()
            ...
            ->limit(1); // Limit to only one customer
        ...
        if ($customer) {
            ...
            // dd($postData);
            // Print data for debugging (could be removed in production)
            echo "Email: " . $email . ", CustomerNo: " . $customerNo . "<br>";
            ...
            if (curl_errno($curl)) {
                ...
                $this->messageManager->addSuccessMessage(__('gg'.$response));
            }

            // Close cURL session
            curl_close($curl);
        } else {
            $this->messageManager->addErrorMessage(__('No customer found to sync.'));
        }
